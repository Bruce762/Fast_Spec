---
name: spec
description: 統一規劃與開發工作流程入口。子指令：discuss（前置討論，嚴禁實作）、proposal（規劃方案）、implement（實施）、verify（驗證實作）、modify（修改方案）、findbug（找 bug）、accomplish（歸檔）、check-architecture（了解架構）、find-why（解釋原因）、sync-readme（同步 README 並維護 CHANGELOG.md）。呼叫格式：/spec [子指令] [提案簡稱]
...
| verify | .claude/skills/spec/verify.md |
