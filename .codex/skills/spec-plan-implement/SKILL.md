...
description: Use when the user invokes `$spec-plan-implement` or asks to implement a named approved plan from `fastplan/plan` and move it to pending test.
---
...
1. 讀取 `@fastplan/plan/[提案簡稱]_plan.md`
   * 若不存在，檢查 `@fastplan/pending_test/[提案簡稱]_plan.md`：若在該處，告知使用者計劃已實作過，可執行 `$spec-plan-verify` 或 `$spec-plan-modify`，直接結束
   * 兩處都不存在 → 告知使用者需先執行 `$spec-plan-proposal [提案簡稱]` 建立計劃，直接結束
...
6. 實作完成後，將 `@fastplan/plan/[提案簡稱]_plan.md` 移動到 `@fastplan/pending_test/`
   ...
   * 若目標已有同名檔案，直接覆蓋
7. 若 `AGENTS.md` 存在，判斷此次實作是否涉及架構變動（新增/移除模組、改變執行指令、新增目錄、修改關鍵設計決策），有則更新對應段落，維持原有結構與風格
...
> 實作完成，計畫已移至 `fastplan/pending_test/`。可執行 `$spec-plan-verify [提案簡稱]` 驗證改動效果。
